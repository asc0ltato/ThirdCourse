...
{
	char cmd[7];		///всегда значение SINC
	long long curvalue;	///тек. значение счетчика времени
};
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	try
	{
		...
		for (int i = 0; i < 10; i++)
		{
			// Время отправки запроса (текущее время)
			GetSystemTime(&tm);
			auto start = chrono::high_resolution_clock::now();

			// Текущее время
			getsincro.curvalue = (long long)chrono::duration_cast<chrono::milliseconds>(start.time_since_epoch()).count();
			strcpy_s(getsincro.cmd, sizeof(getsincro.cmd), "SINC");
			...
			long long rtt = duration_cast<milliseconds>(end - start).count();

			// setsincro.correction - разница во времени между сервером и клиентом
			// rtt / 2 - RTT включает 2 стороны, а реальная разница во времени между К и С
			// должна учитывать задержку в одну сторону
			int correction = setsincro.correction + rtt / 2;
			...
			cout << i + 1 << ") curvalue = " << getsincro.curvalue << " RTT = " << rtt << " correction = " << correction << " max corr/min corr: " << maxcor << "/" << mincor << "\n\n\n";

			if (i == 0)
				getsincro.curvalue += correction;
			else
