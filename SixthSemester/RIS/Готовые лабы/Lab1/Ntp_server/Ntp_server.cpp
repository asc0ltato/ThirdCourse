...
void runServer() 
{
    ...
    if (bind(sS, (LPSOCKADDR)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR) {
    ...
    while (count != 22) {
        ...
        int curvalue = getsincro.correction;
        setsincro.correction = cS - curvalue;

        // Пропускаем первое значение для расчета средней коррекции
        averageCorrection[(count - 2) % 10] = setsincro.correction;

        // Для второго запроса устанавливаем среднюю коррекцию равной первому значению
        if (count == 2) {
            average = 0;
        }
        else {
        ...
        char buffer[10];
        recvfrom(sS, buffer, sizeof(buffer), 0, (struct sockaddr*)&clientAddr, &clientLen);

        uint64_t current_Cs = Cs.load();
        sendto(sS, (char*)&current_Cs, sizeof(current_Cs), 0, (struct sockaddr*)&clientAddr, clientLen);
