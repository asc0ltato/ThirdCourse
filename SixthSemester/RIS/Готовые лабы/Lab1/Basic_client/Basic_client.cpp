...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	try
	{
		...
		// Раскомментируйте, если хотите вывести на клиенте среднюю коррекцию
		//int correctionSum = 0;
		...
		for (int i = 0; i < 10; i++)
		{
			...
			//	avgcorr = correctionSum / correctionCount;
			//}

			cout << tm.wMonth << "." << tm.wDay << "." << tm.wYear << " - " << (tm.wHour + 3) % 24 << ":" << tm.wMinute << ":" << tm.wSecond << "." << tm.wMilliseconds << "\n";
			cout << i + 1 << ") curvalue = " << getsincro.curvalue << " correction = " << correction << " maxcor/mincor: " << maxcor << "/" << mincor << "\n\n\n";

			//cout << tm.wMonth << "." << tm.wDay << "." << tm.wYear << " - " << (tm.wHour + 3) % 24 << ":" << tm.wMinute << ":" << tm.wSecond << "." << tm.wMilliseconds << "\n";
			//cout << i + 1 << ") curvalue = " << getsincro.curvalue << " correction = " << correction << " maxcor/mincor: " << maxcor << "/" << mincor << " average correction = " << avgcorr << "\n\n\n";
