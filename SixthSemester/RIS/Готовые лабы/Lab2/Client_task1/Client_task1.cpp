...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	try
	{
		s = InitCA((char*)"172.20.10.2", (char*)"Z:\\haha.txt", 1); //1?
		// айпи координатора
		...
		while (true)
		{
			...
			bool close = false;
			string line;
			ofstream s1("Z:\\haha.txt", ios::app); 
			ifstream s2("Z:\\haha.txt", ios::app);
			cout << "1. Enter to write.\n2. Enter to read.\n3. leave section and close" << endl;
			cin >> k;
			if (k == 1) {
				// Запись в файл
				EnterCA(s);
				for (int i = 0; i < 5; i++)
				{
					...
					if (s1.is_open())
					{
						s1 << "Client: " << s.clientnumber << " " << asctime(timeinfo) << endl;
					}
					Sleep(1000);
				}
				LeaveCA(s);
...
bool LeaveCA(CA& ca)
{
	...
	try
	{
		...
		while (ca.status != ca.LEAVE)
		{
			...
			cout << "Client leaving critical section, status(LeaveCA): " << GetStatusText(ca.status) << endl;
			Sleep(2000);
		}
		if (closesocket(cC) == SOCKET_ERROR)
			throw SetErrorMsgText("closesocket:", WSAGetLastError());
