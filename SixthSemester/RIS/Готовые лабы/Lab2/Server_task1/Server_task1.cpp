...
map<string, CA> section_map; // Контейнер для хранения данных о ресурсе (по имени ресурса)
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	try
	{
		...
		while (true)
		{
			...
			CA ibuf;  // Буфер для принятого сообщения
			int libuf = 0;
			int lobuf = 0;
			...
			// Проверяем статус клиента и выполняем действия
			if (ibuf.status == ibuf.ENTER || ibuf.status == ibuf.WAIT)
			{
				CA* s = NULL;
				// Если карта не пуста, ищем ресурс по имени
				if (!map.empty())
				{
					s = &((*(map.find(ibuf.resurce))).second); 
				}

				// Если ресурс уже занят, ставим клиента в ожидание
				if (s != NULL)
				{
				...
				else
				{
					...
					map.insert(pair<char*, CA>(ibuf.resurce, ibuf)); // Добавляем клиента в карту
					ibuf.status = ibuf.ENTER; // Обновляем статус клиента
				}
			}
			// Если клиент покидает критическую секцию
			else if (ibuf.status == ibuf.LEAVE)
			{
				cout << "Leave from Critical section " << ibuf.number << endl;
				map.erase(map.find(ibuf.resurce)); // Удаляем информацию о клиенте из карты
				ibuf.status = ibuf.LEAVE; // Обновляем статус
			}
