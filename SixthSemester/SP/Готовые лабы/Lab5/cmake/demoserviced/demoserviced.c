...
int main() {
    ...
    struct stat st;
    // Если путь к исходной директории не задан, завершаем сервис
    if (g_source_dir[0] == '\0') {
        LogError("Startup Error", "SourceDir parameter is missing or empty in configuration file. Service stopping.");
        CloseLogFile();
        exit(EXIT_FAILURE);
    }
    // Если исходная директория не существует, завершаем сервис
    ...
    // Если путь не является директорией, завершаем сервис
    ...
    while (g_running) {
        if (g_reload_config_flag) {
            ...
            // Если параметр SourceDir отсутствует, останавливаем демон
            if (g_source_dir[0] == '\0') {
                LogError("Config Reload Error", "SourceDir is missing after reload. Stopping.");
                g_running = 0;
            }
            // Если директория некорректна после перезагрузки конфигурации, останавливаем демон
...
void ReadConfig() {
    ...
    if (g_reserved_dir[0] == '\0') {
        strncpy(g_reserved_dir, DEFAULT_RESERVED_DIR, sizeof(g_reserved_dir) - 1);
    }
    if (g_source_dir[0] == '\0') {
        strncpy(g_source_dir, DEFAULT_SOURCE_DIR, sizeof(g_source_dir) - 1);
    }
    if (g_copy_interval <= 0) {
        g_copy_interval = DEFAULT_INTERVAL;
    }
}
...
void CreateOrCheckDir(const char* path, const char* dir_type_name) {
    ...
    if (path == NULL || path[0] == '\0') {
        if (strcmp(dir_type_name, "log") == 0) {
            strncpy(path, DEFAULT_LOG_DIR, MAX_PATH - 1);
        }
        else if (strcmp(dir_type_name, "reserved backup") == 0) {
            strncpy(path, DEFAULT_RESERVED_DIR, MAX_PATH - 1);
        }
    }
