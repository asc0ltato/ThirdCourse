...
char fixed_log_dir[MAX_PATH] = "C:\\Users\\ascoltat0\\AppData\\DemoService\\Logs";
...
char reserved_dir[MAX_PATH] = "C:\\Users\\ascoltat0\\AppData\\DemoService\\Reserved";
int copy_interval;
...
VOID WINAPI ServiceMain(DWORD dwArgc, LPTSTR* lpszArgv) {
    ...
    if (!critical_error) {
        ...
        strcpy_s(log_dir_full, MAX_PATH, log_path);
...
void ReadConfig() {
    strcpy_s(reserved_dir, MAX_PATH, "C:\\Users\\ascoltat0\\AppData\\DemoService\\Reserved");
    ...
    char default_log_full[MAX_PATH];
    ...
    if (!PathFileExists(config_path)) {
        ...
        GetPrivateProfileString("Settings", "ReservedDir", "", temp_reserved, MAX_PATH, config_path);

        if (strlen(temp_reserved) == 0) {
        ...
        GetPrivateProfileString("Settings", "LogDir",
            (default_log_full[0] != '\0' ? default_log_full : ""),
            temp_log, MAX_PATH, config_path);

        if (strlen(temp_log) == 0 && default_log_full[0] == '\0') {
            LogError("ReadConfig", "'LogDir' not found in configuration and default path failed. Logs will not be written.");
            strcpy_s(log_path, MAX_PATH, "");
        }
        else if (strlen(temp_log) == 0 && default_log_full[0] != '\0') {
            snprintf(msg_buffer, sizeof(msg_buffer), "Config: 'LogDir' not found. Using default: %s", fixed_log_dir);
            LogInfo(msg_buffer);
            strcpy_s(log_path, MAX_PATH, fixed_log_dir);
        }
        else {
            if (PathIsRelative(temp_log)) {
                PathCombine(log_path, executable_dir, temp_log);
                snprintf(msg_buffer, sizeof(msg_buffer), "Config: Read relative LogDir = %s. Resolved to: %s", temp_log, log_path);
                LogInfo(msg_buffer);
            }
            else {
                strcpy_s(log_path, MAX_PATH, temp_log);
                snprintf(msg_buffer, sizeof(msg_buffer), "Config: Read absolute LogDir = %s", log_path);
                LogInfo(msg_buffer);
            }
        }
    ...
    snprintf(msg_buffer, sizeof(msg_buffer), "Config Loaded: Source='%s', Reserved='%s', Interval=%d min.",
        ...
        log_path[0] != '\0' ? log_path : "[Not Set/Invalid]",
        copy_interval);
    LogInfo(msg_buffer);
}
