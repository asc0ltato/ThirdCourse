...
const getSyndrome = (received, g) => {
    ...
    return rem.toString(2).padStart(5, '0');
};
...
const splitMessage = (msg, size) => {
    ...
    for (let i = 0; i < msg.length; i += size) {
        result.push(msg.slice(i, i + size).padEnd(size, '0'));
    }
    return result;
};

const interleave = (matrix) => {
...
const deinterleave = (str, rows, cols) => {
    const matrix = Array.from({ length: rows }, () => Array(cols).fill('0'));
    let index = 0;
    for (let c = 0; c < cols; c++) {
        for (let r = 0; r < rows; r++) {
            matrix[r][c] = str[index++];
        }
    }
    return matrix.map(row => row.join(''));
};

const addRandomGroupErrors = (bitStr, groupLen) => {
    const bits = bitStr.split('');
    const start = Math.floor(Math.random() * (bits.length - groupLen + 1));
    for (let i = 0; i < groupLen; i++) {
        bits[start + i] = bits[start + i] === '0' ? '1' : '0';
    }
    return bits.join('');
};
