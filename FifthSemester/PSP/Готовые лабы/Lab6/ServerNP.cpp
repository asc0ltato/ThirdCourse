...
int main()
{
    ...
    try {
        if ((sH = CreateNamedPipe(TEXT("\\\\.\\pipe\\Tube"), PIPE_ACCESS_DUPLEX, PIPE_TYPE_MESSAGE | PIPE_WAIT, 1, NULL, NULL, INFINITE, NULL)) == INVALID_HANDLE_VALUE)
        ...
        while (true) {
            ...
            while (running) {
                if (ReadFile(sH, rbuf, sizeof(rbuf), &dwRead, NULL)) {
                    ...
                    if (WriteFile(sH, wbuf, sizeof(wbuf), &dwRead, NULL)) {
