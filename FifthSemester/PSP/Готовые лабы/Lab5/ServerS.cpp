...
void PrintHostName() {
    ...
    if (gethostname(hostName, sizeof(hostName)) == SOCKET_ERROR) {
        throw SetPipeError("gethostname: ", WSAGetLastError());
    }
    cout << "Имя текущего хоста: " << hostName << endl;
}

void PrintClientHostName(SOCKADDR_IN* client) {
    hostent* clientHost;
    clientHost = gethostbyaddr((char*)&client->sin_addr, sizeof(client->sin_addr), AF_INET);

    if (clientHost != nullptr) {
        cout << "Клиент с IP " << inet_ntoa(client->sin_addr) << " имеет имя: " << clientHost->h_name << endl;
    }
    else {
        cout << "Не удалось получить символическое имя клиента с IP " << inet_ntoa(client->sin_addr) << endl;
    }
}
...
bool GetRequestFromClient(char* name, short port, struct sockaddr* from, int* flen) {
    ...
    if ((sS = socket(AF_INET, SOCK_DGRAM, NULL)) == INVALID_SOCKET) {
        throw SetPipeError("socket: ", WSAGetLastError());
    }

    if (setsockopt(sS, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuse, sizeof(reuse)) == SOCKET_ERROR) { //SO_BROADCAST
        throw SetPipeError("setsockopt: ", WSAGetLastError());
    }

    if (bind(sS, (LPSOCKADDR)&serv, sizeof(serv)) == SOCKET_ERROR) {
        throw SetPipeError("bind: ", WSAGetLastError());
    }
    ...
    while (true) {
        if ((len = recvfrom(sS, buf, sizeof(buf), NULL, from, flen)) == SOCKET_ERROR) {
            ...
            if (errCode == WSAETIMEDOUT) {
                ...
                closesocket(sS);
                throw SetPipeError("recvfrom: ", errCode); 
            }
        }
...
bool PutAnswerToClient(char* name, struct sockaddr* to, int* lto) {
    ...
    if ((sS = socket(AF_INET, SOCK_DGRAM, NULL)) == INVALID_SOCKET) {
        throw SetPipeError("socket (PutAnswerToClient): ", WSAGetLastError());
    }

    if (sendto(sS, name, strlen(name) + 1, NULL, to, *lto) == SOCKET_ERROR) {
        closesocket(sS);
        throw SetPipeError("sendto: ", WSAGetLastError());
    }
...
void DiscoverServers(char* name, short port) {
    ...
    if ((sS = socket(AF_INET, SOCK_DGRAM, NULL)) == INVALID_SOCKET) {
        throw SetPipeError("socket (DiscoverServers): ", WSAGetLastError());
    }

    if (setsockopt(sS, SOL_SOCKET, SO_BROADCAST, (char*)&broadcast, sizeof(broadcast)) == SOCKET_ERROR) {
        throw SetPipeError("setsockopt (SO_BROADCAST): ", WSAGetLastError());
    }
...
int main()
{
    ...
    try {
        if (WSAStartup(MAKEWORD(2, 0), &wsaData) != 0) {
            throw SetPipeError("Startup: ", WSAGetLastError());
        }
        ...
        if (WSACleanup() == SOCKET_ERROR) {
            throw SetPipeError("Cleanup: ", WSAGetLastError());
        }
    }
