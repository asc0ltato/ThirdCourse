...
bool GetServer (const char* serverName, char* call, struct sockaddr* from, int* flen) {
    SOCKET cC;
    SOCKADDR_IN serv;
    BOOL broadcast = TRUE;
    char buf[50];
    ...
    if (host == nullptr) {
        cout << "Ошибка: не удается найти символическое имя сервера." << endl;
        return false;
    }

    serv.sin_family = AF_INET;
    serv.sin_port = htons(2000);
    serv.sin_addr.s_addr = *(u_long*)host->h_addr_list[0];
    ...
    try {
        ...
        if (recvfrom(cC, buf, sizeof(buf), 0, from, flen) == SOCKET_ERROR) {
            ...
            throw SetPipeError("recvfrom: ", WSAGetLastError());
        }
...
int main() {
    ...
    try {
        ...
        char call[] = "Hello";
        const char* serverName =  "ASCOLTAT0";

        if (GetServer(serverName, call, (struct sockaddr*)&from, &flen)) {
