...
/*Volatile — ключевое слово языков C/C++, которое информирует компилятор 
о том, что значение переменной может 
меняться из вне и что компилятор не будет оптимизировать эту переменную.*/
...
DWORD WINAPI ConsolePipe(LPVOID pPrm); // Серверная часть консоли управления
DWORD WINAPI GarbageCleaner(LPVOID pPrm); // Сборщик мусора
DWORD WINAPI DispatchServer(LPVOID pPrm); // Диспетчиризация запросов клиента
DWORD WINAPI ResponseServer(LPVOID pPrm); // Ответ сервера клиентам
...
typedef list<Contact> ListContact; // Список подключения клиентов 
...
void openSocket()
{
    ...
    if (ioctlsocket(sS, FIONBIO, &nonblk) == SOCKET_ERROR)
        throw SetErrorMsgText("ioctlsocket:", WSAGetLastError()); // переключает  сокет в режим без блокировки.
}
    ...
    while (cmd != EXIT)
    {
        ...
        case SHUTDOWN:
            ...
            if (previousCommand != SHUTDOWN)
            {
                ...
                printf("size of contacts %d\n", contacts.size());
                sq = 0;
                cmd = EXIT;  // закометить для wait + stop и раскоментить снизу
                //previousCommand = SHUTDOWN;
            }
...
{
    if (!strcmp(param, "START")) return START; // Команда разрешает подключение клиентов к серверу
    if (!strcmp(param, "STOP")) return STOP; // Команда запрещает подключение клиентов к серверу
    if (!strcmp(param, "EXIT")) return EXIT; // Команда завершает работу сервера
    if (!strcmp(param, "WAIT")) return WAIT; // Команда приостанавливает подключение клиентов до тех пор, пока не обслужится последний клиент, подключенный к серверу
    if (!strcmp(param, "SHUTDOWN")) return SHUTDOWN; // Команда равносильна последовательности команд: wait, exit.    
    if (!strcmp(param, "STATISTICS")) return STATISTICS; // Вывод статистики: общее количество подключений; количество активных подключений, количество отказов в обслуживании.
    if (!strcmp(param, "GETCOMMAND")) return GETCOMMAND; // Служебная команда, которая не предназначена для ввода с консоли управления, а устанавливается сервером для указания, что сервер готов принять и обработать, очередную команду управления
    ...
    try
    {
        ...
        while (param != EXIT)
        {
            ...
            while (ReadFile(hPipe, rbuf, sizeof(rbuf), &dwRead, NULL))
            {
                ...
                if (param == EXIT || param == SHUTDOWN) // закоментить  || param == SHUTDOWN если надо чтоб сервер при shutdown 
                {                                       // не принимал сообщения до команды старт (wait + stop)
                    break;
                }
            }
...
int main(int argc, char* argv[]) // главный поток сервера
{
