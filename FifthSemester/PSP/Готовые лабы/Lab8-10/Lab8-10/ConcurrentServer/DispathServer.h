						...
						for (ListContact::iterator p = Contacts.begin(); p != Contacts.end(); p++) {
							if (p->type == Contact::ACCEPT && p->sthread != Contact::ABORT)
							{
								...
								if (flag == true) {//if Rand, Echo or Time
									...
									_int64 time = -300000000; // в наносекундах
									SetWaitableTimer(client->htimer, (LARGE_INTEGER*)&time, 0, ASWTimer, client, false); // ASWTimer - функция завершения,0 - не периодическая
