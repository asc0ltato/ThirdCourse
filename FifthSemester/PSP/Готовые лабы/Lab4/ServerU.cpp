    ...
    try {
        ...
        while (true) {
            ...
            while (true) {
                ...
                if ((libuf = recvfrom(sS, ibuf, sizeof(ibuf), NULL, (sockaddr*)&clnt, &lclnt)) == INVALID_SOCKET) {
                ...
                cout << "Получено сообщение от клиента: " << ibuf << endl;

                strcpy(obuf, ibuf);
                if ((libuf = sendto(sS, obuf, strlen(obuf) + 1, NULL, (sockaddr*)&clnt, lclnt)) == INVALID_SOCKET) {
